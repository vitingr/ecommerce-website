  ...
  useEffect(() => {
    if (query) {
      ...
      if (data.preco !== undefined) {
        ...
        setParcelamento(
          (data.preco / 12).toLocaleString("pt-BR", {
            style: "currency",
            currency: "BRL",
          })
        );
    ...
    <div>
      <div className="w-full h-[100px] p-12 bg-[#222] flex items-center justify-center">
        <div className="flex items-center justify-between max-w-[1200px] w-full">
          <h2 className="text-white text-3xl font-bold w-full flex justify-center">
            {data.nome}
          </h2>
          <div className="flex items-center justify-center gap-6 w-full">
            <div className="flex flex-col text-right w-full">
              <h3 className="text-white font-extralight text-xs">
                Por{" "}
                <span className="font-bold text-base text-white">{preco}</span>{" "}
                à vista
              </h3>
              <h5 className="text-white font-extralight text-xs">
                ou 12x de{" "}
            ...
            <Link
              href={`/buy/${data.subcategoria}-${data.id}/confirm`}
              className="bg-blue-400 text-white rounded-full p-2 w-[200px] text-center cursor-pointer hover:bg-blue-500 transition-all duration-300 font-bold"
            >
              Comprar Produto
            </Link>
          </div>
      ...
      <div className="w-full p-[2%] flex justify-center gap-24 bg-[#f7f7f7]">
        <section className="max-w-[1150px] w-full p-16 bg-white rounded-lg">
          <div>
            <h4 className="flex items-center gap-4 mb-10 text-neutral-500 text-lg cursor-pointer">
              Home <IoCaretForward /> Catalogo <IoCaretForward /> {data.nome}
            </h4>
          </div>
          <div className="flex w-full justify-center">
            <div className="max-w-[500px] w-full flex flex-col items-center">
              <img
                src={data.foto}
                alt="Product Photo"
                className="max-w-[500px] w-full cursor-pointer transition-all duration-300 hover:scale-105"
              />
            </div>
            ...
            <div className="flex flex-col w-full max-w-[650px]">
              <div className="min-h-[250px]">
                ...
                <h4 className="text-neutral-500 text-sm font-normal mt-2">
                  {data.descricao ? (
                    <>{data.descricao}</>
                  ) : (
                    <>
                      Lorem ipsum dolor sit amet consectetur adipisicing elit.
                      Porro at dignissimos consectetur ab enim, magnam incidunt
                      perspiciatis culpa harum ducimus doloremque odit modi
                      soluta est eligendi quas tenetur quod repellendusa.
                    </>
                  )}
                </h4>
                ...
                {data.voltagem ? (
                  <>
                    ...
                    <div className="grid grid-cols-2 gap-6 mt-4">
                      <div className="p-4 border border-zinc-200 rounded-lg flex justify-center text-sm cursor-pointer">
                        110 V
                      </div>
                      <div className="p-4 border border-zinc-200 rounded-lg flex justify-center text-sm cursor-pointer">
                        220 V
              ...
              <div className="rounded-xl">
                ...
                <Link
                  href={`/buy/${data.subcategoria}-${data.id}/confirm`}
                  className="bg-[#262f40] mt-4 cursor-pointer font-bold text-center text-white w-full p-[7.5px] rounded-full hover:bg-[#1d2430] transition-all duration-200"
                >
